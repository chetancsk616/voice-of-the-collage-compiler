// Java AST extractor (Tree-sitter normalized nodes)
...
function extractFromAST(ast, code) {
  ...
  const loopTypes = new Set(['for_statement', 'enhanced_for_statement', 'while_statement', 'do_statement']);
  ...
  walk(ast, (node) => {
    ...
    if (node.text) {
      const t = node.text;
      if (/new\s+(HashMap|LinkedHashMap|HashSet|TreeMap|Hashtable)\s*<|\bMap<|\bSet</.test(t)) features.usesHashMap = true;
      if (/new\s+Stack\s*<|\.push\(|\.pop\(/.test(t)) features.usesStack = true;
      if (/new\s+(Queue|LinkedList)\s*<|\.offer\(|\.poll\(|\.add\(/.test(t)) features.useQueue = true;
